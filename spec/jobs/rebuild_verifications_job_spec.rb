  ...
  describe RebuildVerificationsJob do
    ...
    shared_examples "destroys and rebuilds verification" do
      it "creates the verification" do
        ...
        expect(Decidim::Authorization.find_by(user:, name: workflow_name)).to be_present
      end

      it "rebuids the verification" do
        create(:authorization, name: workflow_name, user:, created_at: 1.day.ago)
        expect(Decidim::Authorization.where(user:, name: workflow_name).count).to eq(1)
        expect { subject.perform_now(workflow_name, organization.id) }.to(change { Decidim::Authorization.find_by(user:, name: workflow_name).created_at })
        expect(Decidim::Authorization.where(user:, name: workflow_name).count).to eq(1)
      end

      context "when other authorizations exist" do
        let!(:dummy_authorization_handler) { create(:authorization, user:, name: "dummy_authorization_handler") }

        it "does not destroy the other authorization" do
          expect(Decidim::Authorization.where(user:, name: workflow_name).count).to eq(0)
          expect { subject.perform_now(workflow_name, organization.id) }.not_to(change { Decidim::Authorization.where(user:, name: "dummy_authorization_handler").count })
          expect(Decidim::Authorization.where(user:, name: workflow_name).count).to eq(1)
        end
      end
    ...
    context "when contact has been updated" do
      ...
      it "creates the verification with the new data" do
        subject.perform_now(workflow_name, organization.id)
        expect(Decidim::Authorization.where(user:, name: workflow_name).count).to eq(1)
        expect(Decidim::Authorization.find_by(user:, name: workflow_name).metadata).to eq({ "contact_id" => 9999, "membership_types_ids" => [1], "uid" => 42 })
      end
    end
    ...
    context "when the workflow_name is not a civicrm authorization" do
      ...
      it "does not create the verification" do
        ...
        expect(Decidim::Authorization.find_by(user:, name: workflow_name)).not_to be_present
      end
    end
  end
end
